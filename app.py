...
def register():
    ...
    try:
        ...
        return jsonify({
            ...
            "redirect": "buchung.html"  # Weiterleitung zur Buchungsseite
        }), 201
    ...
    try:
        ...
        if user:
            return jsonify({
                ...
                "userId": user["id"]  # User-ID zurückgeben
            }), 200
        else:
            return jsonify(
                {"error": "Anmeldung fehlgeschlagen. Überprüfen Sie Ihre Personalnummer und Ihr Passwort."}), 401
...
def booking():
    ...
    try:
        if request.method == "GET":
            ...
            cursor.execute("SELECT * FROM item WHERE id = %s AND status = 'verfügbar' AND battery_level >= 80",
                           (item_id,))
...
def return_item():
    ...
    try:
        ...
        conn.commit()
        return jsonify({"message": "Fahrzeug erfolgreich zurückgegeben.", "redirect": "auswahl.html"}), 200
